...
class Intelipost_Push_Model_Observer 
{
	...
	{
		Mage::helper('push')->log('intelipostGenerateShipment');
		$sucess = false;
		$error_order_ids = array();
		$save_order_comments = Mage::getStoreConfig('intelipost_push/general/save_on_order_comments');
		...
		if ($canUpdate)
		{
			try
			{
				Mage::helper('push')->log('intelipostGenerateShipment update');
				$expression = "main_table.status";
				...
				if (count($collection->getData()) > 0)
		       	...
		       	//		$intelipost_basic_order = Mage::getModel ('basic/orders')->load ($data['entity_id'], 'order_id');
		       			...
		       			Mage::helper('push')->log('generate shipment');
		       			if ($utils->generateShipment ($data['entity_id']))
	                    {
	                    	Mage::helper('push')->log('generate shipment s');

	                        $utils->orderShipped ($data['entity_id']);
	                        Mage::helper('push')->log('order shipped');
	                        $sucess = true;	                       
